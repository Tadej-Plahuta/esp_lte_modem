...
// #include "APN_settings.h"

#define APN_NAME_LENGTH 50
#define APN_USERNAME_LENGTH 50
#define APN_PASSWORD_LENGTH 50

typedef struct
{
    char APN[APN_NAME_LENGTH];
    char username[APN_USERNAME_LENGTH];
    char pasword[APN_PASSWORD_LENGTH];
} APN_settings_struct;
...
static esp_err_t esp_dte_handle_line(esp_modem_dte_t *esp_dte)
{
    ...
    if (len > 2 && !is_only_cr_lf(line, len)) {
        ...
        ESP_LOGI("Modem output", "%s", line); // print AT output from modem
#endif
...
#if MODEM_DEBUG_OUTPUT
    ESP_LOGI("Modem command", "%s", command);
#endif
...
esp_err_t esp_modem_stop_ppp(modem_dte_t *dte)
{
    ...
    /* Enter command mode */
    MODEM_CHECK(dte->change_mode(dte, MODEM_COMMAND_MODE) == ESP_OK, "enter command mode failed - GSM ERROR!!", err);
